...
struct PropertyBlockSetup
{
	...
	{
		inInfo.visitBaseProperties( makePvdPropertyFilter( *this, key ) );
		inInfo.visitInstanceProperties( makePvdPropertyFilter( *this, key ) );
	}
};
...
#undef DEFINE_BASIC_ADD_PROPERTY
	...
	void complexProperty( PxU32* key, const TAccessorType& inAccessor, TInfoType& inInfo )
	{
		...
		inInfo.visitBaseProperties( makePvdPropertyFilter( theSender, key ) );
		inInfo.visitInstanceProperties( makePvdPropertyFilter( theSender, key ) );
	}
};
