...
public:
	...
	*/
	PX_CUDA_CALLABLE PX_INLINE PxQuat(PxReal nx, PxReal ny, PxReal nz, PxReal nw) : x(nx),y(ny),z(nz),w(nw) {}
	...
	*/
	...
	\brief Copy ctor.
	*/
	PX_CUDA_CALLABLE PX_INLINE PxQuat(const PxQuat& v): x(v.x), y(v.y), z(v.z), w(v.w) {}
	...
	*/
	PX_CUDA_CALLABLE PX_INLINE PxReal dot(const PxQuat& v) const
	{
...
//		return (v*(w*w-0.5f) - (qv.cross(v))*w + qv*(qv.dot(v)))*2;
	...
	*/
	...
	PX_CUDA_CALLABLE PX_INLINE PxQuat& operator*= (const PxQuat& q)
	{
