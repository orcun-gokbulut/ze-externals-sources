...
namespace physx { namespace repx {
	...
	inline void nonNullTerminateWhite( char*& ioData )
	{
		if ( ioData )
		{
			while( *ioData && !isspace( *ioData ) )
	...
	inline void strtoLong( PxF32& ioDatatype, char*& ioData )
	{
	...
	inline void strtoShort( PxF32& ioDatatype, char*& ioData )
	{
		//See above.  For short strings, we don't need to worry about null termination.
		//It isn't ideal but it works.

		eatwhite( ioData );
		char* target = ioData;
		nonNullTerminateWhite( ioData );
		ioDatatype = static_cast<PxF32>( strtod( target, &ioData ) );
	}
	inline void strto( PxF32& ioDatatype, char*& ioData )
	{
		strtoShort( ioDatatype, ioData ); 
	}
	...
	inline void strto( physx::PxVec3& ioDatatype, char*& ioData )
	{
		StrToImpl<PxF32>().strtoShort( ioDatatype[0], ioData );
		StrToImpl<PxF32>().strtoShort( ioDatatype[1], ioData );
		StrToImpl<PxF32>().strtoShort( ioDatatype[2], ioData );
	}
	};
	...
	template<typename TDataType>
	inline void strtoLong( TDataType& ioType, char*& ioData )
	{
		if ( ioData && *ioData ) StrToImpl<TDataType>().strto( ioType, ioData );
	}
