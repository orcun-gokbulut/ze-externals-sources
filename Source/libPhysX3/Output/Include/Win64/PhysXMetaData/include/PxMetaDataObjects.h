...
{
	const char* mName;
	PxPropertyInfoName::Enum mKey; 
	PxPropertyInfoBase( const char* n, PxPropertyInfoName::Enum inKey )
		: mName( n )
		, mKey( inKey )
	{
	}
...
template<PxPropertyInfoName::Enum TKey>
...
template<PxPropertyInfoName::Enum TKey, typename TObjType, typename TCollectionType, typename TFilterType>
...
template<PxPropertyInfoName::Enum TKey, typename TObjType, typename TCollectionType, typename TCreateArg>
...
{
	PxGeometryType::Enum getGeometryType(const PxShape* inShape) const;
	...
	bool getGeometry(const PxShape* inShape, PxConvexMeshGeometry& geometry) const;
...
{
	void setMaterials(PxShape* inShape, PxMaterial*const* materials, PxU32 materialCount) const;
};
...
{
	PxShape* createShape(PxRigidActor* inActor, const PxGeometry& geometry, PxMaterial& material, const PxTransform& localPose = PxTransform::createIdentity()) const;
	PxShape* createShape(PxRigidActor* inActor, const PxGeometry& geometry, PxMaterial *const* materials, PxU32 materialCount, const PxTransform& relativePose = PxTransform::createIdentity()) const;
};
...
{
	PxArticulationLink*	createLink(PxArticulation* inArticulation, PxArticulationLink* parent, const PxTransform& pose) const;
};
