...
public:
	...
	enum
	{
		eMAX_SOLVERPREPSPU_BYTESIZE=18896
	};
	enum
	{
		eMAX_SOLVERPRPEP_DATASIZE=4096
	};
#endif

	bool isValid()	const	
	{	
		return connector != 0 && (actor[0]!=0 || actor[1]!=0) && solverPrep != 0;	
	}
	...
	PxConstraintDesc(): 
		...
		linearBreakImpulse(PX_MAX_REAL),
		angularBreakImpulse(PX_MAX_REAL)
	{
		actor[0] = actor[1] = 0;
	}
};
