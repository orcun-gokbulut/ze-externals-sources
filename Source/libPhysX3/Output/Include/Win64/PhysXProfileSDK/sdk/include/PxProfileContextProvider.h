...
#ifndef PX_PHYSX_PROFILE_CONTEXT_PROVIDER_H
#define PX_PHYSX_PROFILE_CONTEXT_PROVIDER_H
...
namespace physx {
	...
	struct PxProfileEventExecutionContext
	{
		PxU32					mThreadId;
		PxU8					mCpuId;
		ThreadPriority::Enum	mThreadPriority;

		PxProfileEventExecutionContext( PxU32 inThreadId = 0, ThreadPriority::Enum inPriority = ThreadPriority::eNORMAL, PxU8 inCpuId = 0 )
	...
	struct PxDefaultContextProvider
	{
		...
		{ 
			...
			ThreadPriority::Enum priority( Thread::getPriority( theId ) );
			PxU8 cpudId = Cpu::getCpuId();
			return PxProfileEventExecutionContext( static_cast<PxU32>( theId ), priority, cpudId );
		}
