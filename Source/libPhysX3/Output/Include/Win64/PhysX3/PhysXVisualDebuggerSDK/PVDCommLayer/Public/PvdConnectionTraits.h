...
namespace PVD
{
	...
	struct PvdConnectionTraits
	{
																		...
																		, PvdConnectionDataProvider* inDataProvider = NULL )
		{
			...
			if ( inCheckAPI )
			{
				TFactoryType* theFactory = AllocateObject<TFactoryType>( inOutStream, inInStream, inConnectionType );
				retval = initializeFactory( theFactory, inHandler, inDataProvider );
			}
			else
			{
				TNoCheckFactoryType* theFactory = AllocateObject<TNoCheckFactoryType>( inOutStream, inInStream, inConnectionType);
				retval = initializeFactory( theFactory, inHandler, inDataProvider );
			}
			return retval;
		}
