...
//@{
	...
	*/
	...
	The PxParticleCreationData descriptor is used to create new particles. For each particle successfully created an index in the range of 
	[0, PxParticleBaseDesc::maxParticles-1] is reserved. These new particle indices are written to the user memory defined by the 
	indexBuffer parameter, unless the user sets indexBuffer to NULL. The new particles can be immediately read from the application readable 
	...
	\param indexBuffer Structure describing where particle indices can be written (if set, it has to be consistent with numParticles).
	\return Number of successfully created particles.

	@see PxParticleCreationData, PxParticleReadData
	*/
	virtual		PxU32						createParticles(const PxParticleCreationData& creationData, const PxStrideIterator<PxU32>* indexBuffer)	= 0;
	...
	*/
	...
	the indices listed in the index buffer. Duplicate indices are allowed. A rest offset buffer of stride zero is allowed.
	Application readable particle data is updated accordingly.
...
//@{
	...
	*/
	...
	virtual		const PxParticleBase*		isPxParticleBase()		const	{ return this; }
