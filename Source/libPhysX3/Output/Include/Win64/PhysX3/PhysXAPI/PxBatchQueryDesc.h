...
/**
\brief Stuct for the result of batched raycast query
*/
struct PxRaycastQueryResult
{
	...
	{
		/**
		\brief Batched raycast query is not start, this is the default status.
		*/
		ePENDING = 0,

		/**
		\brief Batched raycast query is success.
		*/
		eSUCCESS,

		/**
		\brief Batched raycast query is aborted due to the result buffer is full.
		*/
		eABORTED
	};
...
/**
\brief Stuct for the result of batched sweep query
*/
...
/**
\brief Stuct for the result of batched overlap query
*/
...
public:
	...
	*/
	...
	\note For ps3, this must be 16 bytes aligned and not live on stack
	...
	*/
	...
	If the buffer is too small to store hits, the related PxRaycastQueryResult.queryStauts will be set eABORTED
	\note For ps3, this buffer must be 16 bytes aligned and not live on stack
	...
	*/
	...
	If the buffer is too small to store hits, the related PxSweepQueryResult.queryStauts will be set eABORTED
	\note For ps3, this buffer must be 16 bytes aligned and not live on stack
	...
	*/
	...
	If the buffer is too small to store hits, the related PxOverlapQueryResult.queryStauts will be set eABORTED
	\note For ps3, this buffer must be 16 bytes aligned and not live on stack
	...
	*/
	...
	/**
	\brief The size of  user-allocated userRaycastHitBuffer

	*/
	PxU32							raycastHitBufferSize;

	/**
	\brief The size of  user-allocated userSweepHitBuffer

	*/
	PxU32							sweepHitBufferSize;

	/**
	\brief The size of  user-allocated userOverlapHitBuffer
