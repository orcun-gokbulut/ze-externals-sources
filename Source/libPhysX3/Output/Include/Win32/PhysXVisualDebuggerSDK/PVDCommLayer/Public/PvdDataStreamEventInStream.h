...
namespace PVD
{
	...
	public:
		...
		{
			PxU32 theSize = mDatatypeHandlers.size();
			for ( PxU32 idx = 0; idx < theSize; ++idx )
			{
				if ( mDatatypeHandlers[idx]->datatype() == inType )
					return mDatatypeHandlers[idx];
			}
			return NULL;
		}
		...
		void streamify( const char*& outString )
		{
			...
			else
			{
				if ( inLen )
				{
					...
					updateFail( mBlockParser.readBlock<char>( inLen ) );
				}
			}
		}
