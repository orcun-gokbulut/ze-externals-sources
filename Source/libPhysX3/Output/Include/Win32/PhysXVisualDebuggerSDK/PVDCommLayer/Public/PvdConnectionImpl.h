...
#define CHECK_STREAM_CONNECTION( stream ) if ( !stream || !stream->isConnected() ) return PvdCommLayerError::NetworkError
	...
	class PvdConnectionImpl : public InternalPvdConnection, public PvdConnectionDataHandler, public StreamOwnerDisconnectionType
	{
	...
	public:
		PvdConnectionImpl( PvdCommOutStream*	inOutStream, PvdCommInStream* inInStream, TConnectionFlagsType inConnectionType )
			: mStreamOwner( NULL )
			, mRefCount( 0 )
			...
			, mMemoryEventClient( NULL )
		{
			...
			//disable buffering on the consoles for now.
		...
		TLocalConnectionType* doCreateConnection()
		{
			...
			new (theConnection) TLocalConnectionType( theWriter, mTypeChecker );
