...
*/
struct PxControllerAction
{
	enum Enum
	{
		eNONE,				//!< Don't apply forces to touched actor
		ePUSH,				//!< Automatically compute & apply forces to touched actor (push)
	};
};
...
public:
	...
	*/
	...
	\param collisionFlags returned collision flags, collection of ::PxControllerFlag
	...
	\param filterData Alternative filter data used to filter shapes, see PxBatchQuery::overlapAABBMultiple().
	\param filterCallback Custom filter logic to filter out colliding objects.
	*/
