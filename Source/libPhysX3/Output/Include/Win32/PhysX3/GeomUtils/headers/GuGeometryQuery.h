...
namespace Gu
{
	...
	public:
		...
		\param[in] geom0 The geometry object to sweep. Supported geometries are #PxSphereGeometry, #PxCapsuleGeometry and #PxBoxGeometry
		\param[in] globalPose0 Pose in world space of the geometry object to sweep
		\param[in] geom1 The geometry object to test the sweep against
		\param[in] globalPose1 Pose in world space of the geometry object to sweep against
		...
		\param[in] context MeshQueryContext object which needs to be provided for sweeps against #PxConvexMeshGeometry, #PxTriangleMeshGeometry or #PxHeightFieldGeometry objects
		\return True if the swept geometry object geom0 hits the object geom1

		@see SweepHit MeshQueryContext
		*/
		static bool sweep(	const PxVec3& unitDir, 
							const PxReal distance,
							const PxGeometry& geom0,
							const PxTransform& globalPose0,
							const PxGeometry& geom1,
							const PxTransform& globalPose1,
							PxSweepHit& sweepHit,
							PxU32 hintFlags=0xffffffff,
							Gu::MeshQueryContext* context = NULL);
		...
		\param[in] globalPose Pose in world space of the geometry object to sweep.
		...
		@see Triangle TriangleCollisionFlag SweepHit
		*/
		static bool sweep(	const PxVec3& unitDir,
							const PxReal distance,
		...
		\param[in] geom0 The first geometry object. Supported geometries are #PxSphereGeometry, #PxCapsuleGeometry and #PxBoxGeometry
		\param[in] globalPose0 Pose of the first geometry object in world space
		\param[in] geom1 The second geometry object
		\param[in] globalPose1 Pose of the second geometry object in world space
		\param[in] context MeshQueryContext object which needs to be provided for overlap tests vs. #PxTriangleMeshGeometry objects
		\return True if the two geometry objects overlap

		@see MeshQueryContext
		*/
		static bool overlap(const PxGeometry& geom0,
							const PxTransform& globalPose0,
							const PxGeometry& geom1,
							const PxTransform& globalPose1,
							Gu::MeshQueryContext* context = NULL);
		...
		\param[in] globalPose Pose of the geometry object in world space
