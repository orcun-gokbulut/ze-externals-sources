...
namespace PVD
{
	...
	public:
		...
		typedef PvdDataStreamEventOutStream<SU8ContainerWriter<TPXU8Container> > TContainerStream;
	protected:
		...
		TPXU8Container					mEventContainer; //Events get written first to the container, then the output stream.
		...
		SU8ContainerWriter<TPXU8Container> mContainerWriter;
		PvdDataStreamEventOutStream<SU8ContainerWriter<TPXU8Container> > mContainerStream;
	...
	protected:
		...
		{
			//The event size does not include the type and size fields.
			if ( mEventType == PvdConnectionEventType::EventBatch )
			{
				...
				SEventHeader3 theHeader = { mEventType, PX_PROFILE_POINTER_TO_U64( this ), physx::shdfnd::Time::getCurrentCounterValue(), inPayloadSize + 4 };
				theHeader.toBytes( mHeaderStorage );
				writeU32( mHeaderStorage + 21, mEventCount );
				writeBytes( mHeaderStorage, 25 );
			}
			else if ( mEventType == PvdConnectionEventType::MultipleEvent )
			{
				//The total event size is whatever we have buffered along with
				//4 bytes for the event count and one byte for
				//the event type.
				SEventHeader3 theHeader = { mEventType, PX_PROFILE_POINTER_TO_U64( this ), physx::shdfnd::Time::getCurrentCounterValue(), inPayloadSize + 5 };
				theHeader.toBytes( mHeaderStorage );
				writeU32( mHeaderStorage + 21, mEventCount );
				mHeaderStorage[25] = mMultipleType.mEventType;
				writeBytes( mHeaderStorage, 26 );
			}
			return mLastError;
		}
		...
		inline PvdCommLayerError SendContainer()
		{
			...
			{
				ScopedStreamWatcher<TStreamType> watcher( mOutStream );
				SendHeader(theContainerSize);
				PvdCommLayerError retval = PvdCommLayerError::None;
				writeBytes( &mEventContainer[0], mEventContainer.size() );
				mEventCount = 0;
		...
		{
			//Ensure we are in batching mode.
			if ( mEventType != PvdConnectionEventType::EventBatch )
			{
				SendContainer();
				mEventType = PvdConnectionEventType::EventBatch;
		...
		inline PvdCommLayerError BeginMultiple( const TDataType& inDataType
													, PvdConnectionEventType inBlockType )
		{
		...
		{
			if ( !(mMultipleType == inDataType.getEventType()) || mEventType != PvdConnectionEventType::MultipleEvent )
			{
				SendContainer();
				mMultipleType = inType;
				mEventType = PvdConnectionEventType::MultipleEvent;
			}
			//Multiple lists do not contain types, their type is specified only once.
			SerializeToContainer( inDataType, false );
			return mLastError;
		}
